...
Example for running:
0. Cp ckpts to local
aws s3 cp --recursive s3://ai2-llm/checkpoints/OLMoE2/olmoe2-8x1b-newhp-newds-final-annealFrom1200000/step23842 /data/niklas/llm/checkpoints/olmoe2-8x1b-newhp-newds-final-annealFrom1200000_step23842
1. Unshard your OLMoE2 checkpoint using https://github.com/allenai/OLMo/blob/7d63fe09d23cf23714da5aa633a44a90180195da/scripts/unshard.py
...
python OLMo/scripts/unshard.py /data/niklas/llm/checkpoints/23485/step954000 /data/niklas/llm/checkpoints/1b-954000-unsharded --model-only
python OLMo/scripts/unshard.py /data/niklas/llm/checkpoints/olmoe2-8x1b-newhp-newds-final-annealFrom1200000_step23842 /data/niklas/llm/checkpoints/olmoe2-8x1b-newhp-newds-final-annealFrom1200000_step23842-unsharded --model-only
2. Convert to transformers
rm -rf olmoe2; mkdir olmoe2; python /data/niklas/transformers/src/transformers/models/olmoe2/convert_olmoe2_weights_to_hf.py --input_dir /data/niklas/llm/checkpoints/olmoe2-8x1b-newhp-newds-final-annealFrom1200000_step23842-unsharded --tokenizer_json_path /data/niklas/llm/checkpoints/olmoe2-step1200000-unsharded/tokenizer.json --output_dir olmoe2
3. Load model via:
```
from transformers import Olmoe2ForCausalLM, AutoTokenizer
import torch
model = Olmoe2ForCausalLM.from_pretrained("../transformers/olmoe2", torch_dtype=torch.bfloat16).cuda()
model = Olmoe2ForCausalLM.from_pretrained("../transformers/olmoe2").cuda()
tokenizer = AutoTokenizer.from_pretrained("../transformers/olmoe2")
...
import torch
model = OLMo.from_checkpoint("/data/niklas/llm/checkpoints/olmoe2-step1200000-unsharded-pt")
model = model.cuda()
...
from transformers import Olmoe2Config, Olmoe2ForCausalLM
...
def write_model(model_path, input_base_path, tokenizer_path=None, safe_serialization=True, fix_eos_token_id=True):
    ...
    olmoe2_config = yaml.safe_load(config_path.read_text())["model"]

    if fix_eos_token_id:
        olmoe2_config["eos_token_id"] = 50279

    n_layers = olmoe2_config["n_layers"]
    n_heads = olmoe2_config["n_heads"]
    dim = olmoe2_config["d_model"]
    dims_per_head = dim // n_heads
    base = 10000.0
    inv_freq = 1.0 / (base ** (torch.arange(0, dims_per_head, 2).float() / dims_per_head))
    max_position_embeddings = olmoe2_config["max_sequence_length"]

    vocab_size = olmoe2_config.get("embedding_size", olmoe2_config["vocab_size"])

    if olmoe2_config.get("n_kv_heads", None) is not None:
        num_key_value_heads = olmoe2_config["n_kv_heads"]  # for GQA / MQA
    elif olmoe2_config["multi_query_attention"]:  # compatibility with other checkpoints
    ...
    config = Olmoe2Config(
        ...
        pad_token_id=olmoe2_config["pad_token_id"],
        bos_token_id=None,
        eos_token_id=olmoe2_config["eos_token_id"],
        tie_word_embeddings=olmoe2_config["weight_tying"],
        rope_theta=base,
        clip_qkv=olmoe2_config.get("clip_qkv"),
    )
    ...
    print("Loading the checkpoint in a OLMoE2 model.")
    model = Olmoe2ForCausalLM.from_pretrained(tmp_model_path, torch_dtype=torch.bfloat16)
...
def _write_tokenizer(
    output_path: Path, config: Olmoe2Config, input_tokenizer_path: Path, fix_eos_token_id: bool = True
) -> None:
...
def main():
    ...
    parser.add_argument(
        "--input_dir",
        required=True,
        help="Location of OLMoE2 weights, which contains config.yaml and model.pt.",
    )
    parser.add_argument(
        "--tokenizer_json_path",
        default=None,
        help="Location of OLMoE2 tokenizer json file.",
    )
    parser.add_argument(
        "--output_dir",
        required=True,
